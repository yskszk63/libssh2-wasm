...
export type Libssh2Exports = {
  ...
  libssh2_knownhost_readline(hosts: number, line: number, len: number, type: number): number
  libssh2_knownhost_checkp(hosts: number, host: number, pot: number, key: number, keylen: number, typemask: number, knownhost: number): number
}
